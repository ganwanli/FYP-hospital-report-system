...
-- 创建外键约束（如果相关表存在）
-- 注意：如果相关表不存在，请注释掉相应的外键约束

-- 模板ID外键约束
ALTER TABLE sql_template_version 
ADD CONSTRAINT fk_sql_template_version_template_id 
FOREIGN KEY (template_id) REFERENCES sql_template(template_id) 
ON DELETE CASCADE ON UPDATE CASCADE;

-- 创建人外键约束（假设用户表名为sys_user，如果不同请修改）
-- ALTER TABLE sql_template_version 
-- ADD CONSTRAINT fk_sql_template_version_created_by 
-- FOREIGN KEY (created_by) REFERENCES sys_user(user_id) 
-- ON DELETE RESTRICT ON UPDATE CASCADE;

-- 审批人外键约束
-- ALTER TABLE sql_template_version 
-- ADD CONSTRAINT fk_sql_template_version_approved_by 
-- FOREIGN KEY (approved_by) REFERENCES sys_user(user_id) 
-- ON DELETE RESTRICT ON UPDATE CASCADE;

-- 父版本外键约束
ALTER TABLE sql_template_version 
ADD CONSTRAINT fk_sql_template_version_parent_version 
FOREIGN KEY (parent_version_id) REFERENCES sql_template_version(version_id) 
ON DELETE SET NULL ON UPDATE CASCADE;

-- 创建唯一索引，确保每个模板只有一个当前版本
-- 注意：MySQL 8.0+ 支持函数索引，如果是较老版本可能需要调整
CREATE UNIQUE INDEX uk_template_current_version 
ON sql_template_version (template_id) 
WHERE is_current = TRUE;

-- 如果上面的唯一索引创建失败，可以使用触发器来保证约束
-- 或者在应用层面保证逻辑

-- 插入示例数据（可选，如果sql_template表中有对应数据）
-- 请根据实际的sql_template表数据调整template_id
...
-- 查看插入的示例数据
SELECT version_id, template_id, version_number, version_description, is_current, created_time 
FROM sql_template_version 
ORDER BY template_id, created_time;
